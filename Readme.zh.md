...
本框架实现了标准 PFC 模型及其扩展（二元合金、三维 BCC/FCC/SC 晶格、Swift-Hohenberg 模型），采用高效的半隐式傅里叶谱方法进行数值求解。模块化架构将核心求解器、分析工具、可视化和输入输出分离为独立的 mixin 类，支持灵活的功能组合和易于扩展的开发模式。
核心能力
表格
类别	功能
...
PFCAnalysis3D	三维结构因子、peak_local_max原子检测、二维切片
...
标准PFC模拟的入口点：
调用 input_pfc_parameters() 进行交互式配置
使用用户参数实例化 PurePFCSolver
运行主模拟循环 + 高级分析
执行 postprocess()（10图套件）
运行 analyze_psi6()（完整取向序分析）
空位检测与扩散动画
...
系统的弹性模量测量流程：
弛豫 PurePFCSolver 至平衡态（T=1500）
保存参考状态
施加13个应变值：linspace(-0.03, 0.03, 13)
每个应变点弛豫2000步
拟合 F(ε) = a·ε² + b·ε + c → 提取 C = 2a
绘制能量-应变和应力-应变曲线
run_3d.py — 三维模拟运行脚本
交互式模式选择的三维模拟入口：
表格
