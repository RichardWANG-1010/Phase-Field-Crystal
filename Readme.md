...
Calls input_pfc_parameters() for interactive config
Instantiates PurePFCSolver with user parameters
Runs main simulation loop + advanced analysis
Executes postprocess() (10-plot suite)
Runs analyze_psi6() (full orientational order analysis)
Vacancy detection and diffusion animation
...
Equilibrates PurePFCSolver (T=1500)
Saves reference state
Applies 13 strain values: linspace(-0.03, 0.03, 13)
Relaxes 2000 steps per strain point
Fits F(ε) = a·ε² + b·ε + c → extracts C = 2a
Plots energy-strain and stress-strain curves
...
ψ₆ Order	Bond orientational order (`	ψ6	= 1perfect,0` disordered)
